...
import edu.osu.cse.presto.android.gator.gui.intent.IntentAnalysisInfo;
...
public class Util {
  ...
  protected final Hierarchy hier = Hierarchy.v();
  ...
  public static synchronized Util v() {
    if (util == null) {
      util = new Util();
  ...
  public boolean isMenuItemAddCall(Stmt s) {
    ...
    SootClass declz = mtd.getDeclaringClass();
    String mtdSig = mtd.getSubSignature();
    boolean is = util.hier.isMenuClass(declz);
    if (!is) {
      return false;
    }
  ...
  public boolean isIntentAllocNode(NNode node) {
    ...
    return util.isIntentType(initType);
  }
  ...
  public void readFromSpecificationFile(String fn) {
    Logger.verb(Util.class.getSimpleName(), "Reading specs from " + fn);
  ...
  protected void readByType(String type, Node role) {
    if (type.equals("startActivity")) {
      ...
      Logger.err(getClass().getSimpleName(), "reading " + Configs.wtgSpecFile
  ...
  public boolean isAsyncMethodCall(Stmt s) {
    ...
    for (SootClass clz : asyncOperationMethods.keySet()) {
      if ((Configs.asyncStrategy == Configs.AsyncOpStrategy.Default_EventHandler_Async
              || Configs.asyncStrategy == Configs.AsyncOpStrategy.Default_Special_Async)
    ...
    for (int i = 0; i < classes.getLength(); i++) {
      ...
      for (int j = 0; j < invocations.getLength(); j++) {
        ...
        for (int k = 0; k < args.getLength(); k++) {
          ...
          subsigPair.put(subsig, new Pair<>(Integer.parseInt(cxtPos), Integer.parseInt(intentPos)));
        }
      }
    }
  }
