...
import edu.osu.cse.presto.android.gator.Debug;
import edu.osu.cse.presto.android.gator.MethodNames;
import edu.osu.cse.presto.android.gator.MultiMapUtil;
import edu.osu.cse.presto.android.gator.gui.flowgraph.nodes.*;
import edu.osu.cse.presto.android.gator.gui.listener.EventType;
import edu.osu.cse.presto.android.gator.gui.listener.ListenerRegistration;
...
import edu.osu.cse.presto.android.gator.gui.util.JimpleUtil;
...
public class Flowgraph implements MethodNames {
  ...
  public Map<Integer, NDrawableIdNode> allNDrawableIdNodes = Maps.newHashMap();
  ...
  public Map<Integer, NIntegerConstantNode> allNIntegerConstantNodes = Maps.newHashMap();

  public edu.osu.cse.presto.android.gator.Hierarchy hier;
  public Set<Integer> allDrawableIds;
  ...
  // Utils
  protected JimpleUtil jimpleUtil;
  protected GraphUtil graphUtil;
  public XMLParser xmlUtil;


  public Flowgraph(edu.osu.cse.presto.android.gator.Hierarchy hier, Set<Integer> allLayoutIds, Set<Integer> allMenuIds, Set<Integer> allWidgetIds, Set<Integer> allStringIds, Set<Integer> allDrawableIds) {
    this(hier, allLayoutIds, allMenuIds, allWidgetIds, allStringIds);
    this.allDrawableIds = allDrawableIds;
  }

  public Flowgraph(edu.osu.cse.presto.android.gator.Hierarchy hier, Set<Integer> allLayoutIds, Set<Integer> allMenuIds, Set<Integer> allWidgetIds, Set<Integer> allStringIds) {
    this.hier = hier;
  ...
  protected void processFrameworkManagedCallbacks() {
  ...
  Map<SootClass, Map<SootMethod, Set<SootMethod>>> activityToCreateOrPrepareMenuAndItemSelected =
      Maps.newHashMap();
  ...
  void modelOnCreateOrPrepareOptionsMenuAndItsFlowToItemSelected(SootClass activityClass) {
    ...
    Set<String> itemSelectedSubsigs =
        Sets.newHashSet(onOptionsItemSelectedSubSig, onMenuItemSelectedSubSig);
      ...
      for (Iterator<SootMethod> iter = c.getMethods().iterator(); iter.hasNext(); ) {
        ...
        while (stmts.hasNext()) {
          ...
          if (currentStmt.containsInvokeExpr()) {
            ...
            for (Iterator<SootClass> trg_i =
                 hier.getConcreteSubtypes(stc).iterator(); trg_i.hasNext(); ) {
  ...
  protected void resolveArrayRefs() {
  ...
  public void recordVarAtArrayRefRead(Local x, Stmt s) {
  ...
  public void recordVarAtArrayRefWrite(Local x, Stmt s) {
  ...
  public NOpNode createInflate1OpNode(Stmt s) {
    ...
    if (outsideRoot == null) {
      ...
      Local fakeLocal =
          Jimple.v().newLocal(fakeLocalName, Scene.v().getSootClass("android.view.View").getType());
      ...
      NOpNode addView2 =
          new NAddView2OpNode(outsideRootNode, fakeLocalNode, new Pair<Stmt, SootMethod>(s, jimpleUtil.lookup(s)), false);
      allNNodes.add(addView2);
      if (Configs.debugCodes.contains(Debug.OP_NODE_DEBUG)) {
        System.out.println(
            "[AddView2] parent: " + outsideRootNode + ", child: " + fakeLocalNode + " @ "
                + addView2);
    ...
    if (Configs.debugCodes.contains(Debug.OP_NODE_DEBUG)) {
      System.out.printf("[Inflate1] id: %s, lhs: %s, outside: %s @ %s\n", layoutIdNode.toString(), (
          lhsNode == null ?
              "<NULL>" :
              lhsNode.toString()), (outsideRoot == null ?
          "<NULL>" :
          varNode(outsideRoot).toString()), inflate1.shortDescription());
  ...
  public NInflate2OpNode createInflate2OpNode(Stmt s) {
    ...
    boolean isActivity =
        hier.libActivityClasses.contains(c) || hier.applicationActivityClasses.contains(c);
    boolean isDialog =
        hier.libraryDialogClasses.contains(c) || hier.applicationDialogClasses.contains(c);
    ...
    NInflate2OpNode inflate2 =
        new NInflate2OpNode(layoutIdNode, receiverNode, new Pair<Stmt, SootMethod>(s, jimpleUtil.lookup(s)), false);
  ...
  public NOpNode createFindView1OpNode(Stmt s) {
    ...
    NOpNode findView1 =
        new NFindView1OpNode(widgetIdNode, receiverNode, lhsNode, new Pair<Stmt, SootMethod>(s, jimpleUtil.lookup(s)),
            menuFindItem ?
                NFindView1OpNode.FindView1Type.MenuFindItem :
                NFindView1OpNode.FindView1Type.Ordinary, false);

    if (Configs.debugCodes.contains(Debug.OP_NODE_DEBUG)) {
      System.out.println(
          "[FindView1] view: " + receiverNode + ", id: " + widgetIdNode + ", lhs: " + lhsNode
  ...
  public NOpNode createFindView2OpNode(Stmt s) {
    ...
    NOpNode findView2 =
        new NFindView2OpNode(layoutIdNode, receiverNode, lhsNode, new Pair<Stmt, SootMethod>(s, caller), false);

    if (Configs.debugCodes.contains(Debug.OP_NODE_DEBUG)) {
      System.out.println(
          "[FindView2] id: " + layoutIdNode + ", act: " + receiverNode + ", lhs: " + lhsNode + " @ "
  ...
  public NOpNode createFindView3OpNode(Stmt s) {
    ...
    NOpNode findView3 =
        new NFindView3OpNode(receiverNode, lhsNode, new Pair<Stmt, SootMethod>(s, jimpleUtil.lookup(s)), type, false);

    if (Configs.debugCodes.contains(Debug.OP_NODE_DEBUG)) {
      System.out.println("[FindView3] view: " + receiverNode + ", lhs: " + lhsNode + " @ "
          + findView3.shortDescription());
      System.out.println("  stmt: " + s + " @ " + jimpleUtil.lookup(s));
    }
  ...
  public NAddView1OpNode createAddView1OpNode(Stmt s) {
    ...
    NAddView1OpNode addView1 =
        new NAddView1OpNode(parameterNode, receiverNode, new Pair<Stmt, SootMethod>(s, jimpleUtil.lookup(s)), false);
  ...
  public NOpNode createSetIdOpNode(Stmt s) {
    ...
    NOpNode setId =
        new NSetIdOpNode(idNode, receiverNode, new Pair<Stmt, SootMethod>(s, jimpleUtil.lookup(s)), false);
  ...
  public NOpNode createSetListenerOpNode(Stmt s) {
    ...
    if (isRegisterForContextMenu) {
      ...
      if (listenerPosition == ListenerSpecification.UNKNOWN_POSITION) {
        System.out.println(
            "[WARNING] Something is wrong for listenerPosition at " + s + " @ " + caller);
        return null;
      }
    ...
    boolean isContextMenuSetListener =
        name.equals(setOnCreateContextMenuListenerName) || isRegisterForContextMenu;
    SootClass listenerParameterType;
    if (isContextMenuSetListener) {
      listenerParameterType =
          Scene.v().getSootClass("android.view.View$OnCreateContextMenuListener");
    } else {
  ...
  public boolean processSetListenerOpNode(NSetListenerOpNode opNode, NObjectNode viewObject, NObjectNode listenerObject) {
    ...
    for (SootMethod h : handlers) {
      ...
      if (!valid) {
        System.out.println(
            "[WARNING] Cannot find View parameter for " + h.getSignature() + ", listenerObject: "
                + listenerObject + ", viewObject: " + viewObject + ", reason: " + s + " in "
                + caller);
        continue;
      }
  ...
  NSetListenerOpNode createSetListenerAndProcessFlow(Local viewLocal, Local listenerLocal, Stmt s, SootMethod caller, boolean isContextMenuSetListener, ListenerRegistration registration, SootClass listenerClass, SootClass listenerParameterType, boolean shouldProcessFlow, EventType eventType, /* optional */
    ...
    NSetListenerOpNode setListener =
        new NSetListenerOpNode(viewNode, listenerNode, callSite, isContextMenuSetListener, artificial);
  ...
  void processFlowFromSetListenerToEventHandlersInFuture(SootClass listenerClass, SootClass listenerParameterType, NVarNode listenerNode, NVarNode viewNode, NSetListenerOpNode setListener, Stmt s, SootMethod caller, ListenerRegistration registration, boolean isContextMenuSetListener) {
    FlowFromSetListenerToEventHandlers t =
        new FlowFromSetListenerToEventHandlers(listenerClass, listenerParameterType, listenerNode, viewNode, setListener, s, caller, registration, isContextMenuSetListener);
    tasks.add(t);
  }
  ...
  void processFlowFromSetListenerToEventHandlers(SootClass listenerClass, SootClass listenerParameterType, NVarNode listenerNode, NVarNode viewNode, NSetListenerOpNode setListener, Stmt s, SootMethod caller, ListenerRegistration registration, boolean isContextMenuSetListener) {
    ...
    for (SootMethod h : handlers) {
      ...
      if (!valid) {
        System.out.println(
            "[WARNING] Cannot find View parameter for " + h.getSignature() + ", listenerClass: "
                + listenerClass + ", listenerParameter: " + listenerParameterType + ", reason: " + s
                + " in " + caller);
    ...
    for (SootClass possibleListenerType : listenerTypes) {
      for (SootMethod prototype : handlerPrototypes) {
        ...
        SootClass matchedClass =
            hier.matchForVirtualDispatch(prototypeSubsig, possibleListenerType);
  ...
  public NOpNode createAddMenuItemOpNode(Stmt s) {
    ...
    if (s instanceof DefinitionStmt) {
      ...
      lhsLocal =
          Jimple.v().newLocal(menuItemName, Scene.v().getSootClass("android.view.MenuItem").getType());
    }
  ...
  public NOpNode createMenuItemSetTitleOpNode(Stmt s) {
    ...
    boolean isMenuItemSetTitle =
        menuItemSetTitleCharSeqSubSig.equals(subsig) || menuItemSetTitleIntSubSig.equals(subsig);
  ...
  public NOpNode createGetTabHostOpNode(Stmt s) {
    ...
    if (!(layoutIdNode instanceof NLayoutIdNode)) {
      throw new RuntimeException(
          "[Error]: we can not create a LayoutIdNode from id: " + layoutId.intValue());
    }

    NOpNode inflate2 =
        new NInflate2OpNode(layoutIdNode, receiverNode, new Pair<Stmt, SootMethod>(s, jimpleUtil.lookup(s)), true);
    ...
    NOpNode findView2 =
        new NFindView2OpNode(hostValNode, receiverNode, lhsNode, new Pair<Stmt, SootMethod>(s, jimpleUtil.lookup(s)), false);
  ...
  protected void checkAndPatchRootlessActivities() {
  ...
  protected void checkAndPatchRootlessDialogs() {
    // TODO
  }

  protected void patchListActivity() {
    ...
    for (NActivityNode activityNode : allNActivityNodes.values()) {
      ...
      if (matched != null && matched.isApplicationClass()) {
        ...
        NFindView3OpNode findView3 =
            new NFindView3OpNode(listViewNode, listItemNode, null, NFindView3OpNode.FindView3Type.FindChildren, true);
        allNNodes.add(findView3);

        // A fake listener that calls onListItemClick
        SootClass fakeListenerClass =
            createFakeOnItemClickListenerClass(activityNode, listViewNode, listItemNode, onListItemClick);
        ...
        NSetListenerOpNode setListener =
            createSetListenerAndProcessFlow(listView, fakeListenerLocal, fakeListenerReg, constructor, false, null, fakeListenerClass, Scene.v().getSootClass("android.widget.AdapterView$OnItemClickListener"), true, EventType.item_click, true);
  ...
  SootClass createFakeOnItemClickListenerClass(NActivityNode listActivityNode, NVarNode listViewNode, NVarNode listItemNode, SootMethod handlerMethod) {
    ...
    SootMethod fakeOnItemClickMethod =
        new SootMethod("onItemClick", Lists.<Type>newArrayList(adapterViewType, viewType, IntType.v(), LongType.v()), VoidType.v());
    ...
    // r1 := parent
    Stmt defineParent =
        jimple.newIdentityStmt(parentLocal, jimple.newParameterRef(adapterViewType, 0));
    ...
    // r3 := position
    Stmt definePosition =
        jimple.newIdentityStmt(positionLocal, jimple.newParameterRef(IntType.v(), 2));
    ...
    Expr callOnItemClickExpr =
        jimple.newVirtualInvokeExpr(listActivityLocal, handlerMethod.makeRef(), Lists.newArrayList(parentLocal, viewLocal, positionLocal, idLocal));
   ...
   */
  protected void processRecordedListViewCalls() {
    if (Configs.verbose) {
  ...
  public void connectListViewWithGetView(Stmt setAdapterCall) {
    ...
    for (NNode src : sources) {
      ...
      SootClass concreteType =
          hier.matchForVirtualDispatch(getViewSubSig, ((NObjectNode) src).getClassType());
      ...
      if (concreteType.isApplicationClass()) {
        ...
        if (idNodes == null || idNodes.isEmpty()) {
          System.out.println(
              "[WARNING] Missing layout id for adapter at " + setAdapterCall + " in " + caller);
        } else {
  ...
  public Value extractLayoutIdFromAdapterConstructor(SootMethod method) {
    ...
    while (stmts.hasNext()) {
      ...
      if (call.isConstructor()) {
        ...
        boolean callingSuperConstructor =
            thisClass.hasSuperclass() && callClass.equals(thisClass.getSuperclass());
        ...
        if (Configs.verbose) {
          System.out.println(
              "[WARNING] Failed to extract layout id from " + s + " in " + jimpleUtil.lookup(s));
        }
        return null;
      }
    }
   ...
   */ public Map<Stmt, Local> explicitShowContextMenuCallAndViewLocals = Maps.newHashMap();
  public Map<Stmt, Local> explicitShowOptionsMenuCallAndActivityLocals = Maps.newHashMap();
  ...
  protected void processAllRecordedDialogCalls() {
  ...
  void processRecordedActivityDialogCalls(Map<Local, Set<Stmt>> activityLocalAndCalls, Map<NDialogNode, Set<Stmt>> dialogObjectAndCalls, boolean isShow) {
    for (Map.Entry<Local, Set<Stmt>> entry : activityLocalAndCalls.entrySet()) {
      ...
      if (!activityResolved) {
        System.out.println("[WARNING] Cannot find activity node for " + oneStmt + " @ "
            + jimpleUtil.lookup(oneStmt));
      } else if (!dialogResolved) {
        System.out.println("[WARNING] Cannot find dialog node for " + oneStmt + " @ "
            + jimpleUtil.lookup(oneStmt));
      }
    } // look at input
  }
  ...
  void handleAlertDialogSetView(Local viewLocal, NDialogNode alertDialog, Pair<Stmt, SootMethod> callSite) {
    ...
    NVarNode custom =
        internalDialogFindViewById(alertDialog, "custom", "android.widget.FrameLayout", callSite);
  ...
  void modelListenerForAlertDialogButtons(Local listenerArg, NDialogNode alertDialog, String buttonId, int which, Pair<Stmt, SootMethod> callSite) {
    ...
    for (NNode n : graphUtil.backwardReachableNodes(varNode(listenerArg))) {
      ...
      if (matched != null) {
        ...
        NVarNode button =
            internalDialogFindViewById(alertDialog, buttonId, "android.widget.Button", callSite);
        ...
        SootClass fakeOnClickListenerClass =
            createFakeOnClickListenerClass(alertDialog, dialogInterfaceOnClickListener, handlerMethod, which);
        ...
        NSetListenerOpNode setListener =
            createSetListenerAndProcessFlow(button.l, fakeListenerLocal, s, caller, false, null, fakeOnClickListenerClass, Scene.v().getSootClass("android.view.View$OnClickListener"), true, EventType.click, true);
        allNNodes.add(setListener);
      }
    }
  }
  ...
  SootClass createFakeOnClickListenerClass(NDialogNode alertDialog, NVarNode dialogInterfaceOnClickListener, SootMethod handlerMethod, int which) {
    ...
    RefType viewType = RefType.v("android.view.View");
    SootMethod fakeOnClickMethod =
        new SootMethod("onClick", Collections.<Type>singletonList(viewType), VoidType.v());
    Jimple jimple = Jimple.v();
    ...
    Expr callOnClickExpr =
        jimple.newVirtualInvokeExpr(dialogListenerLocal, handlerMethod.makeRef(), alertDialogLocal, IntConstant.v(which));
    ...
    SootMethod dismissMethod =
        Scene.v().getSootClass("android.app.Dialog").getMethodByName("dismiss");
  ...
  NVarNode internalDialogFindViewById(NObjectNode alertDialog, String viewId, String viewClassName, Pair<Stmt, SootMethod> callSite) {
    ...
    Local fakeReceiverLocal =
        Jimple.v().newLocal(fakeReceiverName, RefType.v(alertDialogClassName));
  ...
  boolean recordTabHostAddTab(Stmt s) {
    ...
    if (!receiverClass.getName().equals("android.widget.TabHost")) {
      System.out.println("[WARNING] Unexpected receiver type " + receiverClass + " for " + s + " @ "
          + jimpleUtil.lookup(s));
      return true;
    }
  ...
  boolean recordTabHostNewTabSpec(Stmt s) {
    ...
    Local lhs = jimpleUtil.lhsLocal(s);
    NTabSpecNode tabSpecNode =
        tabSpecNode(Scene.v().getSootClass("android.widget.TabHost$TabSpec"), s, jimpleUtil.lookup(s));
  ...
  protected void processTabHostRelatedCalls() {
  ...
  void processTabSpecNode(NTabSpecNode tabSpecNode) {
    ...
    for (Stmt setLabelIndicator : MultiMapUtil.getNonNullHashSetByKey(resolvedTabSpecLabelIndicators, tabSpecNode)) {
      ...
      Pair<Stmt, SootMethod> callSite =
          new Pair<Stmt, SootMethod>(setLabelIndicator, jimpleUtil.lookup(setLabelIndicator));
    ...
    for (Stmt setLayoutContent : MultiMapUtil.getNonNullHashSetByKey(resolvedTabSpecLayoutContents, tabSpecNode)) {
      ...
      NVarNode viewNode = varNode(fakeLocal);
      Pair<Stmt, SootMethod> callSite =
          new Pair<Stmt, SootMethod>(setLayoutContent, jimpleUtil.lookup(setLayoutContent));
      NInflate1OpNode infalte1 = new NInflate1OpNode(layoutIdNode, viewNode, callSite, true);
      allNNodes.add(infalte1);
  ...
  void processAddTabCall(NTabSpecNode tabSpecNode, Stmt addTab) {
    ...
    NFindView1OpNode findTabWidget =
        new NFindView1OpNode(tabsIdNode, tabHostNode, mTabWidgetNode, callSite, NFindView1OpNode.FindView1Type.Ordinary, true);
    ...
    Local mTabContent =
        Jimple.v().newLocal(nextFakeName(), RefType.v("android.widget.FrameLayout"));
    ...
    NFindView1OpNode findTabContent =
        new NFindView1OpNode(tabContentIdNode, tabHostNode, mTabContentNode, callSite, NFindView1OpNode.FindView1Type.Ordinary, true);
  ...
  public HashMap<NContextMenuNode, SootMethod> contextMenuToOnCreateContextMenus = Maps.newHashMap();
  ...
  NContextMenuNode findOrCreateContextMenuNode(SootMethod onCreateContextMenuMethod, NVarNode actualViewNode) {
    ...
    if (!subsig.equals(onCreateContextMenuSubSig)) {
      throw new RuntimeException(
          "Unexpected context menu creation method " + onCreateContextMenuMethod);
    }
  ...
  public NContextMenuNode findOrCreateContextMenuNode(SootMethod onCreateContextMenuMethod, NVarNode formalMenuNode, NVarNode actualViewNode) {
  ...
  void modelFlowFromCreateOrPrepareMenuToItemSelected(NNode menuNode, SootClass menuClass, SootClass activityClass, SootMethod createOrPrepareMenuMethod, SootMethod itemSelectedMethod, int menuItemPosition) {
    ...
    Local findItemReceiverLocal =
        Jimple.v().newLocal(findItemReceiverLocalName, menuClass.getType());
    ...
    Local findItemResultLocal =
        Jimple.v().newLocal(findItemResultLocalName, Scene.v().getSootClass("android.view.MenuItem").getType());
    ...
    NOpNode findView3 =
        new NFindView3OpNode(findItemReceiverVarNode, findItemResultVarNode, null, NFindView3OpNode.FindView3Type.FindChildren, true);
    allNNodes.add(findView3);
    // item = lhs
  ...
  public NDrawableIdNode drawableIdNode(Integer i) {
    NDrawableIdNode x = allNDrawableIdNodes.get(i);
    if (x != null) {
      return x;
    }
    x = new NDrawableIdNode(i);
    allNDrawableIdNodes.put(i, x);
    allNNodes.add(x);
    return x;
  }
  ...
  public Map<Integer, NAnonymousIdNode> anonymousIdNodes = Maps.newHashMap();
  ...
  public NIntegerConstantNode integerConstantNode(Integer value) {
    Preconditions.checkNotNull(value);
    NIntegerConstantNode intConstantNode = allNIntegerConstantNodes.get(value);
    if (intConstantNode == null) {
      intConstantNode = new NIntegerConstantNode();
