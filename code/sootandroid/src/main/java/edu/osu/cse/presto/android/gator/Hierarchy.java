...
public class Hierarchy {
  private final String TAG = Hierarchy.class.getSimpleName();
  ...
  public SootMethod virtualDispatch(SootMethod staticTarget,
    ...
    return virtualDispatch(staticTarget.getSubSignature(), receiverClass);
  }
...
//          + ", subsig: " + staticTargetSubsig);
      Logger.verb(TAG, "Hierarchy.virtualDispatch called with non-concrete receiver class: " + receiverClass
              + ", subsig: " + staticTargetSubsig);
      return null;
    }
    // look up the method
    ...
    Logger.verb(TAG,
            "No match in Hierarchy.virtualDispatch: \n\tmethod subsig = "
                    + staticTargetSubsig + "\n\ttype = " + receiverClass);
    return null;
  }
...
//    dialogs();
  ...
  void simpleClassStatistics() {
    ...
    Logger.info(TAG, "All classes: " + numClasses
            + " [App: " + appClasses.size()
            + ", Lib : " + scene.getLibraryClasses().size()
  ...
  void activityClasses() {
    ...
    final SootClass act = Scene.v().getSootClass("android.app.Activity");
    if (act == null) {
      throw new Error("[HIER] Did not find Activity");
    }
    Stream<SootClass> stream = appClasses.parallelStream();
    stream.forEach(new Consumer<SootClass>() {
      @Override
      public void accept(SootClass sootClass) {
        if (getSupertypes(sootClass).contains(act)) {
          applicationActivityClasses.add(sootClass);
        }
      }
    });
    stream = Scene.v().getLibraryClasses().parallelStream();
    stream.forEach(new Consumer<SootClass>() {
      @Override
      public void accept(SootClass sootClass) {
        if (getSupertypes(sootClass).contains(act)) {
          libActivityClasses.add(sootClass);
        }
      }
    });
    Logger.info(TAG, "Activities: " + applicationActivityClasses.size()
  ...
  void viewsAndMenus() {
    ...
    Logger.info(TAG, "App views: " + numAppViews + ", Lib views: "
            + numLibViews);
  }
  ...
  void dialogs() {
    ...
    Logger.info(TAG, "App Dialogs: " + applicationDialogClasses.size()
  ...
  public Set<SootClass> applicationActivityClasses = Collections.newSetFromMap(new ConcurrentHashMap<>());
  public Set<SootClass> libActivityClasses = Collections.newSetFromMap(new ConcurrentHashMap<>()); // not in app
