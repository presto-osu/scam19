...
import edu.osu.cse.presto.android.gator.gui.GUIAnalysis;
...
public class AnalysisEntrypoint {
  ...
  public void run() {
    System.out.println("[Stat] #Classes: " + Scene.v().getClasses().size() + ", #AppClasses: "
    ...
    if (Configs.guiAnalysis) {
      GUIAnalysis.v().run();
    } else if (Configs.transformation) {
      Transformation.v().run();
    }
  ...
  void validateMethodNames() {
    ...
    SootClass layoutInflater = Scene.v().getSootClass("android.view.LayoutInflater");
    ...
    assertTrue(MethodNames.setContentViewSubSig.equals(setContentView1.getSubSignature()), "MethodNames.setContentViewSubSig is incorrect.");
    SootMethod setContentView2 = activity.getMethod(MethodNames.setContentViewViewSubSig);
    assertTrue(MethodNames.setContentViewViewSubSig.equals(setContentView2.getSubSignature()), "MethodNames.setContentViewViewSubSig is incorrect.");
    ...
    SootMethod inflate1 =
        layoutInflater.getMethod("android.view.View inflate(int,android.view.ViewGroup)");
    assertTrue(MethodNames.layoutInflaterInflate.equals(inflate1.getSignature()), "MethodNames.layoutInflaterInflate is incorrect.");
    SootMethod inflate2 =
        layoutInflater.getMethod("android.view.View inflate(int,android.view.ViewGroup,boolean)");
    assertTrue(MethodNames.layoutInflaterInflateBool.equals(inflate2.getSignature()), "MethodNames.layoutInflaterInflateBool is incorrect.");

    SootMethod inflate3 =
        view.getMethod("android.view.View inflate(android.content.Context,int,android.view.ViewGroup)");
    assertTrue(MethodNames.viewCtxInflate.equals(inflate3.getSignature()), "MethodNames.viewCtxInflate is incorrect.");

    SootMethod findView1 = view.getMethod("android.view.View findViewById(int)");
    assertTrue(MethodNames.viewFindViewById.equals(findView1.getSignature()), "MethodNames.viewFindViewById is incorrect.");
    SootMethod findView2 = activity.getMethod("android.view.View findViewById(int)");
    assertTrue(MethodNames.actFindViewById.equals(findView2.getSignature()), "MethodNames.actFindViewById is incorrect.");

    SootMethod findView3 = activity.getMethod(MethodNames.findViewByIdSubSig);
    assertTrue(MethodNames.findViewByIdSubSig.equals(findView3.getSubSignature()), "MethodNames.findViewByIdSubSig is incorrect.");

    SootMethod setId = view.getMethod(MethodNames.setIdSubSig);
    assertTrue(MethodNames.setIdSubSig.equals(setId.getSubSignature()), "MethodNames.setIdSubSig is incorrect.");

    // MethodNames.addViewName is fine

    SootMethod findFocus1 = view.getMethod(MethodNames.findFocusSubSig);
    assertTrue(MethodNames.findFocusSubSig.equals(findFocus1.getSubSignature()), "MethodNames.findFocusSubSig is incorrect.");

    // Dialogs
    SootClass dialogInterfaceOnCancel =
        Scene.v().getSootClass("android.content.DialogInterface$OnCancelListener");
    assertTrue(!dialogInterfaceOnCancel.isPhantom(), "DialogInterface.OnCancelListener is phantom!");

    SootMethod dialogInterfaceOnCancelMethod =
        dialogInterfaceOnCancel.getMethod(MethodNames.dialogOnCancelSubSig);
    assertTrue(MethodNames.dialogOnCancelSubSig.equals(dialogInterfaceOnCancelMethod.getSubSignature()), "MethodNames.dialogOnCancelSubSig is incorrect.");
    ...
    assertTrue(!dialogInterfaceOnShow.isPhantom(), "DialogInterface.OnShowListener is phantom!");
