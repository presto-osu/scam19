...
import edu.osu.cse.presto.android.gator.gui.flowgraph.nodes.wtg.*;
import edu.osu.cse.presto.android.gator.gui.util.GraphUtil;
import edu.osu.cse.presto.android.gator.gui.util.QueryHelper;
...
public class IntentAnalysis {
  ...
  private static synchronized void createMotherIntentAnalysis(
          GUIAnalysisOutput guiOutput, Util util) {
    ...
    motherIntentAnalysis.util = util;
    motherIntentAnalysis.initialize(guiOutput);
    motherIntentAnalysis.run();
  }

  public IntentAnalysis(GUIAnalysisOutput guiOutput) {
    util = Util.v();
    initialize(guiOutput);
  }

  public IntentAnalysis(GUIAnalysisOutput guiOutput, Util util) {
    this.util = util;
    initialize(guiOutput);
  }
  ...
  private void initialize(GUIAnalysisOutput guiOutput) {
    ...
    createMotherIntentAnalysis(guiOutput, util);
  ...
  private void run() {
    ...
    IntentFilterReader intentFilterReader = IntentFilterReader.v(util);
  ...
  public IntentAnalysisInfo getIntentAnalysisInfo(NAllocNode intentNode) {
    return approximateIntentContent.get(intentNode);
  }
  ...
  private void prepare() {
    ...
    for (Map.Entry<Expr, NAllocNode> entry : guiOutput.getFlowgraph().allNAllocNodes.entrySet()) {
      ...
      if (!util.isIntentAllocNode(intentAllocNode) && !util.isCreateIntentAllocNode(intentAllocNode)) {
        continue;
      }
  ...
  private void createIntentForLauncherActivity() {
    ...
    Expr newExpr = Jimple.v().newNewExpr(RefType.v(util.intentClass));
...
//      if (((NIntentNode) intentAllocNode).allocMethod.toString().contains("testNotification")) {
//        Logger.info("aaaaaaaaaaaaa", "aaaaaaaaaaaaaaaaaa");
//      }
    ...
    while (!workingList.isEmpty()) {
      ...
      for (Map.Entry<NNode, Set<IntentField>> paraNodeToFields : setIntentContent.getContentMap().entrySet()) {
        ...
        for (IntentField field : fields) {
          ...
          Set<NNode> paraBackReachedNodes = queryHelper.allVariableValues(paraNode);
          for (NNode backReachedNode : paraBackReachedNodes) {
            if (util.isCreateIntentAllocNode(backReachedNode) || util.isIntentAllocNode(backReachedNode)) {
  ...
  private boolean rebuildPropagation() {
    ...
    for (Map.Entry<NAllocNode, IntentAnalysisInfo> allocToContent : intentContent.entrySet()) {
      ...
      if (!intentFlowtoStartActivity.containsKey(intent) && !util.isLauncherIntent(intentInfo)) {
        continue;
      }
      ...
      for (String tgtName : tgtActivities) {
        ...
        SootClass tgtClz = util.getActivitySootClassByNameSig(tgtName);
...
//          if ("nodomain.freeyourgadget.gadgetbridge.activities.charts.LiveActivityFragment$1".equals(actName)) {
//            Logger.info("hailong", ".......................");
//          }
